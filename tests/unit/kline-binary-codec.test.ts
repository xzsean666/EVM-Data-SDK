...
import { KlineBinaryCodec, KLINE_RECORD_SIZE } from "../../src/price/archive/KlineBinaryCodec";
...
describe("KlineBinaryCodec", () => {
  ...
  it("supports O(log N) binary search slicing with startMs and endMs", () => {
    ...
    // Out of range (before)
    const slice3 = KlineBinaryCodec.decode(buffer, 100, 500);
    expect(slice3).toEqual([]);

    // Out of range (after)
    const slice4 = KlineBinaryCodec.decode(buffer, 6000, 8000);
    expect(slice4).toEqual([]);

    // Single point [3000, 3001)
    const slice5 = KlineBinaryCodec.decode(buffer, 3000, 3001);
    expect(slice5.map((p) => p.timestamp)).toEqual([3000]);
  });

  it("handles empty buffer gracefully", () => {
    const decoded = KlineBinaryCodec.decode(Buffer.alloc(0));
