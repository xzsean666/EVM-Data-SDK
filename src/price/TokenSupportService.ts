...
import type { TokenSupportStore } from "../storage/TokenSupportStore";
...
export class TokenSupportService {
  ...
  constructor(
    private readonly store: TokenSupportStore,
  ...
  /**
   * Preloads all spot symbols from Binance and Gate into memory and SQLite.
        ...
        (async () => {
          try {
            ...
            if (res.status === 200 && res.body && typeof res.body === "object") {
              const symbols = (res.body as { symbols?: Array<{ symbol?: string; status?: string; isSpotTradingAllowed?: boolean }> }).symbols;
              if (Array.isArray(symbols)) {
                for (const s of symbols) {
                  if (typeof s.symbol === "string" && s.symbol.endsWith("USDT") && s.status === "TRADING" && s.isSpotTradingAllowed !== false) {
   ...
   * Checks: Memory -> SQLite -> Upstream (with strict 1s timeout).
   * Timeouts and uncertain responses are NOT cached to SQLite.
   */
  async isTokenSupported(
    token: string,
    provider: TokenSupportProvider,
    signal?: AbortSignal,
  ): Promise<boolean> {
    await this.initialize();
  ...
  private async probeBinance(token: string, signal?: AbortSignal): Promise<boolean> {
    try {
      ...
      if (response.status === 200 && response.body && typeof response.body === "object") {
        const symbols = (response.body as { symbols?: Array<{ symbol?: string; status?: string; isSpotTradingAllowed?: boolean }> }).symbols;
        const market = symbols?.find((s) => s.symbol === `${token}USDT` && s.status === "TRADING" && s.isSpotTradingAllowed !== false);
      ...
      // Any other status: uncertain, DO NOT save to SQLite
      return false;
    } catch (error: any) {
      // If HTTP 400 with code -1121 (Invalid symbol), it is a definite negative
      if (error?.response?.status === 400 || error?.status === 400) {
        this.setMemoryCache(token, "binance", false);
        this.store.set(token, "binance", false);
        return false;
      }
      // Timeout or network error: UNCERTAIN, DO NOT save to SQLite
      return false;
    }
  }
  ...
  private async probeGate(token: string, signal?: AbortSignal): Promise<boolean> {
    try {
      ...
      if (response.status === 404) {
        // Not found is a definitive negative answer
