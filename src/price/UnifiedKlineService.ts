...
import type { GateAdapter } from "../providers/price/gate/GateAdapter";
import { EvmDataError } from "../domain/errors";
import { fetchBinanceFiveMinuteKlines } from "../providers/price/binance/binanceFiveMinute";
...
export class UnifiedKlineService {
  ...
  private async fetchKlinesForProvider(
    ...
    if (request.startMs < currentMonthStartMs) {
      ...
      for (const [year, month] of months) {
        ...
        try {
          ...
          // If archive download fails for a month, throw or attempt REST fallback
          throw error;
        }
      }
    }
    ...
    if (request.endMs > currentMonthStartMs) {
      ...
      const recentPoints = provider === "binance"
