...
export function classifyGateResponse(response: HttpResponse): EvmDataError | null {
...
export function normalizeGateTransportError(value: unknown): EvmDataError | null {
...
function error(code: EvmDataError["code"], message: string, retryable: boolean, retryAfterMs?: number): EvmDataError {
  return new EvmDataError({ code, message, retryable, provider: "gate", ...(retryAfterMs === undefined ? {} : { retryAfterMs }) });
}
