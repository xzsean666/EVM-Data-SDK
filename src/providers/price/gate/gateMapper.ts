...
export function mapGateCandles(
  ...
  for (const raw of rows) {
    ...
    points.set(date, Object.freeze({
      ...
      price: close,
      volume: quoteVolume,
      isFinal: confirmed === "true" || confirmed === true ? true : confirmed === "false" || confirmed === false ? false : date < today,
    }));
  }
  return Object.freeze([...points.values()].sort((left, right) => left.date.localeCompare(right.date)));
}

export function gateResult(request: NormalizedTokenPriceRequest, points: readonly TokenPricePoint[]): TokenPriceProviderResult {
