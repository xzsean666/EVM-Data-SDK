...
export const GATE_SPOT_ENDPOINTS = Object.freeze([
  "https://api.gateio.ws",
]);
...
export class GateAdapter implements TokenPriceProviderAdapter {
  ...
  constructor(options: GateAdapterOptions = {}) {
    ...
    const envEndpoints = (process.env.GATE_API_BASE_URLS?.trim() || "")
      .split(",")
      .map((value) => value.trim().replace(/\/$/, ""))
      .filter(Boolean);
    const configured = options.baseUrls ?? (options.baseUrl ? [options.baseUrl] : (envEndpoints.length > 0 ? envEndpoints : GATE_SPOT_ENDPOINTS));
    this.baseUrls = Object.freeze([...new Set(configured.map((value) => normalizeBaseUrl(value, options.allowInsecureHttp ?? false)))]);
  ...
  async getPriceHistory(
    ...
    for (const [startDate, endDate] of chunks(request.resolvedRange.startDate, request.resolvedRange.endDate, CANDLE_CHUNK_DAYS)) {
      ...
      const body = await this.call("/api/v4/spot/candlesticks", {
        currency_pair: pair,
        interval: "1d",
        from: fromSec,
        to: toSec,
  ...
  private async call(
    ...
    throw normalizeGateTransportError(lastError) ?? new EvmDataError({
