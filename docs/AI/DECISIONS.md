...
## ADR-023: Advanced proxy uses a managed sing-box loopback runtime (Superseded)

**Status:** Superseded / Revoked by ADR-036 on 2026-09-19
...
7. Persist active cooldown states into SQLite (`sdk_cooldown_states`) via `CooldownStore`. On `client.initialize()`, reload persisted states into memory pools. Any subsequent failure or recovery updates SQLite atomically, preserving stepped escalation and downtime accumulation across process restarts and scheduled cron runs.
...
## ADR-036: Unified HTTP Proxy-Only Transport Architecture (Revocation of sing-box/VLESS)
...
**Decision:**
1. Completely remove `sing-box` child process runtime, binary downloading/verification, loopback config generation, and non-standard protocol parsing (`vless://`, `vmess://`, `ss://`, etc.).
2. Unify the SDK proxy transport around standard HTTP/HTTPS proxies (`http_proxy` / `https_proxy`) managed by a shared `ProxyPool`.
3. Standardize on a unified "Proxy-Only" transport architecture:
   - When proxies are configured or proxy mode is enabled, `allowDirect: false` is strictly enforced across all outbound services (`RequestExecutor`, `PriceRequestExecutor`, `ApiChainService`).
   - Direct network fallback is strictly prohibited when proxy mode is active. If proxies are exhausted or in cooldown, requests fail fast with `PROXY_ERROR`.
   - `EvmDataClient` instantiates and shares a single `ProxyPool` across executors, synchronizing proxy leases, fair rotation, and cooldown states globally.
4. Keep the direct-only boundary for Archive RPC intact per ADR-028 (Chainlink / DeFi JSON-RPC archive endpoints never use proxies to prevent token egress).

**Reason:**
A managed `sing-box` subprocess added substantial complexity (cross-platform binary downloading, zip/tar.gz checksum verification, file system permissions, ephemeral configuration files, port binding, and process lifecycle management). In practice, production deployments use standard HTTP/HTTPS forward or CONNECT proxies. Unifying the proxy architecture to standard HTTP proxies eliminates child process risks, reduces binary bloat, avoids local disk and network side-effects during prewarming, and provides consistent proxy-only egress guarantees.
