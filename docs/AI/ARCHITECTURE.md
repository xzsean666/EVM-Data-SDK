...
HTTP proxy route from the shared `ProxyPool` as the rest of the client, so
`allowDirect: false` strictly enforces proxy-only routing for chain-height, internal-transfer,
...
- sing-box process runtime and VLESS/SS parsing: rejected and removed in favor of standard unified HTTP ProxyPool.
...
EthereumArchiveRpcPool      (initialize() probes, passive health, random permutation)
       |
       v
ArchiveRpcTransport  ---- direct-only, proxy: null, never ProxyPool
       |
       v
