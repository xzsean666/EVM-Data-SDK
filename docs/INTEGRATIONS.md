...
- Wallet ERC-20 balances use `GET /{address}/erc20` with `chain` and required `to_block`. This is a REST Data API endpoint, not a JSON-RPC call. A proxy-only live check on 2026-08-06 returned an unpaged JSON array containing `token_address`, `balance`, and `decimals`; a request without `to_block` and a malformed `to_block` each returned HTTP 400. The public `client.token.getHoldingsAtBlock({ chain, address, blockNumber })` operation passes the requested block directly and exposes the complete validated array without accepting token addresses. The older `getErc20TokenHoldings` operation remains current-holdings discovery and resolves an indexed head when needed.
- The observed historical wallet-balance response has no cursor and does not honor the normal list `limit`. The adapter validates the complete array, rejects duplicate contract entries, and the address-only operation never fabricates zeroes or projects onto a caller-supplied contract set. The separate `getErc20BalancesAtBlock` operation may project a complete response onto an explicit contract set; an error or malformed response never fabricates zeroes.
